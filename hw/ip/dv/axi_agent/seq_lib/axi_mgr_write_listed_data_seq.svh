...
// per beat, in order.
//
// Use this for multi-beat write bursts (for example a 2-beat CHERI capability write). The caller
// fills m_items with one axi_write_data_item per beat before starting the sequence;
// m_number_of_items is constrained to match the list length.
...
class axi_mgr_write_listed_data_seq extends axi_mgr_write_data_seq;
  ...
  // Index of the next beat to send (advanced by randomize_item on each call).
