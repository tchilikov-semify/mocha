...
#(
  ...
  // Integer operands — connect to alu's internal operand_a / operand_b wires
  // (these are already extracted from the capability register by the ALU via
  // reg_to_x, so no further extraction is needed here).
  ...
  // Sampling gate
  //
  // The ALU has no valid/ready handshake.  We gate sampling on the FU
  // selector: when fu_i == ALU the issue stage has dispatched an ALU
