...
class cva6_rvfi_monitor extends uvm_monitor;
  protected virtual cva6_rvfi_if vif;

  uvm_analysis_port#(cva6_rvfi_seq_item) item_collected_port;

  `uvm_component_utils(cva6_rvfi_monitor)
  `uvm_component_new
  ...
  function void build_phase(uvm_phase phase);
    ...
    if(!uvm_config_db#(virtual cva6_rvfi_if)::get(this, "", "rvfi_if", vif)) begin
      `uvm_fatal("NOVIF", {"virtual interface must be set for: ", get_full_name(), ".vif"});
    end
  endfunction: build_phase

  virtual task run_phase(uvm_phase phase);
    cva6_rvfi_seq_item trans_collected;

    wait (vif.monitor_cb.reset === 1'b0);

    forever begin
      logic [63:0] mem_wmask_bits;
      logic [63:0] mem_rmask_bits;

      // Wait for a retired instruction
      while(!vif.monitor_cb.valid) @(vif.monitor_cb);

      // Read instruction details from RVFI interface
      trans_collected           = cva6_rvfi_seq_item::type_id::create("trans_collected");
      ...
      mem_wmask_bits = {{8{vif.monitor_cb.mem_wmask[7]}},
                        {8{vif.monitor_cb.mem_wmask[6]}},
                        {8{vif.monitor_cb.mem_wmask[5]}},
                        {8{vif.monitor_cb.mem_wmask[4]}},
                        {8{vif.monitor_cb.mem_wmask[3]}},
                        {8{vif.monitor_cb.mem_wmask[2]}},
                        {8{vif.monitor_cb.mem_wmask[1]}},
                        {8{vif.monitor_cb.mem_wmask[0]}}};

      mem_rmask_bits = {{8{vif.monitor_cb.mem_rmask[7]}},
                        {8{vif.monitor_cb.mem_rmask[6]}},
                        {8{vif.monitor_cb.mem_rmask[5]}},
                        {8{vif.monitor_cb.mem_rmask[4]}},
                        {8{vif.monitor_cb.mem_rmask[3]}},
