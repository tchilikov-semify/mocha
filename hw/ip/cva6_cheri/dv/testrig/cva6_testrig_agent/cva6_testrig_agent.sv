...
// UVM agent to connect a CVA6 simulation to TestRIG. TestRIG provides instructions as DII (direct
...
//
// This agent contains a driver (cva6_testrig_dii_driver) which is responsible for handling all of
// the DII packets directly which include things such as reset and interrupts, along with
// instructions. The agent uses cva6_rvfi_monitor to gather retired instruction data from RVFI and
...
class cva6_testrig_agent extends uvm_agent;
  `uvm_component_utils(cva6_testrig_agent)

  cva6_rvfi_monitor rvfi_monitor;
  cva6_testrig_dii_driver dii_driver;
  ...
  uvm_tlm_analysis_fifo #(cva6_rvfi_seq_item) rvfi_port;
  ...
  virtual function void build_phase(uvm_phase phase);
    ...
    rvfi_monitor = cva6_rvfi_monitor::type_id::create("rvfi_monitor", this);
    dii_driver = cva6_testrig_dii_driver::type_id::create("dii_driver", this);
  ...
  task watch_for_reset();
    ...
    forever begin
      ...
      if (rvfi_num_seen != 0 && !hold_rvfi_send) begin
        ...
        // either of these cases something may have gotten out of sync.
  ...
  function testrig_rvfi_exec_pkt_t exec_pkt_from_rvfi(cva6_rvfi_seq_item seq_item);
    return '{
