...
#(
    ...
    logic [                63:0]    alignment_buf;                              // The alignment buffer to re-align instructions based on fetch offset
    logic [CVA6Cfg.DIIIDLEN-1:0]    dii_id, dii_id_ff;                          // the DII ID of the current written instruction
    logic                           is_compressed, is_compressed_next;          // whether the current/next instruction is compressed
    ...
    always @(dii_id_ff, dreq_o.valid, dreq_i, vaddr_ff, fetch_misalign_ff) begin
        ...
        dii_id                  = dii_id_ff + (is_compressed & ~compress_misalign_ff & ~((fetch_misalign ^ fetch_misalign_ff) | realign));
        halt                    = (dii_id > (dii_vif.num_test_insns() + (fetch_misalign | compress_misalign))) && dreq_o.valid;
        instr                   = dii_vif.get_instr(dii_id_ff);
        if(is_compressed) begin
            instr = {dii_vif.get_instr(dii_id_ff + 1'b1)[15:0], dii_vif.get_instr(dii_id_ff)[15:0]};
        end
        alignment_buf = {instr, instr_ff};
    end
    ...
    always_ff @(posedge clk_i or negedge rst_ni) begin
        if (~rst_ni) begin
            ...
            if(dreq_o.valid || kill) begin
                ...
                instr_ff                <= is_compressed ? (dii_vif.get_instr(dii_id_ff+1) << (is_compressed_next * 16)) : instr;
            end
    ...
    always_comb begin
        ...
        dreq_o.data     = (alignment_buf << ((fetch_misalign | compress_misalign_ff) * 16)) >> 32;
