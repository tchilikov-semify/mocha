...
// SPDX-License-Identifier: Apache-2.0
  ...
  class axi_sram_base_vseq extends uvm_sequence;
    ...
    protected task automatic run_write(axi_mgr_write_burst_vseq seq, string ctx);
      bit in_range;
      seq.m_aw_req.m_addr = m_base + seq.m_aw_req.m_addr;   // SRAM-relative -> system address
      in_range   = addr_in_sram(seq.m_aw_req.m_addr);
      ...
      if (in_range) begin
        if (seq.rsp.m_write_response.m_resp != axi_write_response_item::BRespOkay)
          `uvm_error(get_full_name(), $sformatf("%s @0x%0h: non-OKAY BRESP %0d", ctx, seq.m_aw_req.m_addr,
                     seq.rsp.m_write_response.m_resp))
        if (m_check_resp_id && seq.rsp.m_write_response.m_id != seq.m_aw_req.m_id)
          `uvm_error(get_full_name(), $sformatf("%s: BID 0x%0h != AWID 0x%0h", ctx,
                     seq.rsp.m_write_response.m_id, seq.m_aw_req.m_id))
    ...
    protected task automatic run_read(axi_mgr_read_burst_vseq seq, string ctx,
      ...
      bit in_range;
      seq.m_ar_req.m_addr = m_base + seq.m_ar_req.m_addr;   // SRAM-relative -> system address
      in_range   = addr_in_sram(seq.m_ar_req.m_addr);
      ...
      foreach (seq.m_read_beats[i]) begin
        if (in_range) begin
          ...
          if (m_check_resp_id && seq.m_read_beats[i].m_id != seq.m_ar_req.m_id)
            `uvm_error(get_full_name(), $sformatf("%s: beat %0d RID 0x%0h != ARID 0x%0h", ctx, i,
                       seq.m_read_beats[i].m_id, seq.m_ar_req.m_id))
    ...
    protected task automatic write_word(bit [31:0] id, bit [63:0] addr, bit [63:0] data);
      ...
      seq.m_aw_req.m_id = id; seq.m_aw_req.m_addr = addr; seq.m_aw_req.m_size = 3'd3; seq.m_aw_req.m_burst = BurstFixed;
    ...
    protected task automatic write_cap(bit [31:0] id, bit [63:0] addr,
      ...
      seq.m_aw_req.m_id = id; seq.m_aw_req.m_addr = addr; seq.m_aw_req.m_size = 3'd3; seq.m_aw_req.m_burst = BurstIncr;
    ...
    protected task automatic write_burst_words(bit [31:0] id, bit [63:0] addr,
      ...
      seq.m_aw_req.m_id = id; seq.m_aw_req.m_addr = addr; seq.m_aw_req.m_size = 3'd3;
      seq.m_aw_req.m_burst = (words.size() == 1) ? BurstFixed : BurstIncr;
      foreach (words[i])
    ...
    protected task automatic read_word(bit [31:0] id, bit [63:0] addr, output bit [63:0] data);
      ...
      seq.m_ar_req.m_id = id; seq.m_ar_req.m_addr = addr; seq.m_ar_req.m_size = 3'd3; seq.m_ar_req.m_len = 8'd0;
      seq.m_ar_req.m_burst = BurstFixed;
    ...
    protected task automatic read_cap(bit [31:0] id, bit [63:0] addr,
      ...
      seq.m_ar_req.m_id = id; seq.m_ar_req.m_addr = addr; seq.m_ar_req.m_size = 3'd3; seq.m_ar_req.m_len = 8'd1;
      seq.m_ar_req.m_burst = BurstIncr;
    ...
    protected task automatic write_word_strb(bit [31:0] id, bit [63:0] addr, bit [63:0] data,
      ...
      seq.m_aw_req.m_id = id; seq.m_aw_req.m_addr = addr; seq.m_aw_req.m_size = size; seq.m_aw_req.m_burst = BurstFixed;
    ...
    protected task automatic read_generic(bit [31:0] id, bit [63:0] addr, int unsigned nbeats,
      ...
      seq.m_ar_req.m_id = id; seq.m_ar_req.m_addr = addr; seq.m_ar_req.m_size = size; seq.m_ar_req.m_len = 8'(nbeats - 1);
      seq.m_ar_req.m_burst = (nbeats == 1) ? BurstFixed : BurstIncr; seq.m_ar_req.m_prot = prot;
