...
// SPDX-License-Identifier: Apache-2.0
  ...
  class axi_sram_virtual_sequencer extends uvm_sequencer;
    ...
    // Passive clk_rst_if on the agent's own clock/reset domain (published by the
    // axi_bfm), used to observe when the DUT-side reset has settled -- harness-
    // agnostic, no DUT hierarchy path.
    virtual clk_rst_if         agent_rst_vif;

    // SRAM aperture base: SRAMBase (integration) or 0 (block). Set by the harness.
    bit [63:0] sram_base = top_pkg::SRAMBase;
