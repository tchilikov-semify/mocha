...
// SPDX-License-Identifier: Apache-2.0
  ...
  class axi_sram_burst_last_vseq extends axi_sram_base_vseq;
    ...
    task body();
      ...
      rseq.m_ar_req.m_id = 32'd4; rseq.m_ar_req.m_addr = addr; rseq.m_ar_req.m_size = 3'd3;
      rseq.m_ar_req.m_len = 8'(nbeats - 1); rseq.m_ar_req.m_burst = BurstIncr;
  ...
  // wrong_region_decerr -- accesses outside the SRAM aperture return DECERR from
  // the xbar err_slv, confirming only the SRAM aperture reaches axi_sram.
  // Restricted to unmapped gap addresses (TlCrossbar end .. DRAM base): on the full
  // chip the named device regions route to real/external ports (some tied off in
  // this bring-up tb) and do not reliably DECERR. TODO: revisit vs the final vplan.
  class axi_sram_wrong_region_decerr_vseq extends axi_sram_base_vseq;
    ...
    task body();
      bit [63:0] data;
      bit [63:0] addrs[] = '{64'h6800_0000, 64'h7000_0000, 64'h7800_0000};
      m_base = 64'h0;   // absolute system addresses
      await_reset();
      foreach (addrs[i]) begin
        read_word (32'd2, addrs[i], data);
        write_word(32'd2, addrs[i], 64'hA5A5_5A5A_A5A5_5A5A);
      end
      `uvm_info(get_full_name(),
                "wrong_region_decerr: unmapped non-SRAM addresses returned DECERR -- PASS", UVM_LOW)
    endtask
  endclass
